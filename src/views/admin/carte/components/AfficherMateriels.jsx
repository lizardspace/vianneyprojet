...
import { ModalCloseButton, Box, Text, SimpleGrid, VStack, Badge, Alert, AlertIcon, IconButton, Tooltip, Button, Modal, ModalOverlay, ModalContent, ModalHeader, ModalBody, ModalFooter, useDisclosure, useToast, HStack, Select, Input, InputGroup, InputLeftElement } from '@chakra-ui/react';
...
const AfficherMateriels = ({ materialId }) => {
    ...
    useEffect(() => {
        const fetchEvents = async () => {
            ...
            if (error) {
                ...
                setEvents(data);
                // Trouver le id de l'événement actuellement sélectionné
    ...
    useEffect(() => {
        if (materialId) {
          setSearchTerm(materialId); // Update the search term with the material ID when it is passed as a prop
        }
      }, [materialId]);
        ...
        const chargerMateriels = async () => {
            ...
            if (materielsError) {
                ...
                if (teamsError) {
                    ...
                    // Mettre à jour les données des matériels avec les ids des équipes associées
        ...
        <Box padding="4">
            ...
            <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} spacing="20px">
                {materiels.filter((materiel) => {
                    ...
                    // Vérifier si le terme de recherche est inclus dans le id ou la description
                    return materiel.id.toLowerCase().includes(searchTermLower) ||
                    ...
                    <Box key={materiel.id} p="4" shadow="md" borderWidth="1px" borderRadius="md" bg="white">
                        <VStack spacing="4">
                            <Badge colorScheme="orange">{materiel.id}</Badge>
                            <Alert status={materiel.associated_team_id ? "success" : "warning"} variant="left-accent">
                                <AlertIcon />
                                {materiel.associated_team_name ? `Le matériel "${materiel.nom}" est associé à l'équipe "${materiel.associated_team_name}"` : `Aucune équipe n'est associée au matériel "${materiel.id}". Matériel libre.`}
                            </Alert>
                            ...
                            <VStack spacing={4} align="stretch">
                                ...
                                {selectedMaterial ? (
                                    <Badge colorScheme="green" p="2">
                                        {selectedMaterial.id} (Sélectionné)
                                    </Badge>
                                ) : (
