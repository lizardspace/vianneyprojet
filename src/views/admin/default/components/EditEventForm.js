...
export default function EditEventForm({ event, refreshEvents }) {
    ...
    const [eventDate, setEventDate] = useState(event.event_date); // Date in "YYYY-MM-DD" format
      ...
      const { error } = await supabase.from('vianney_event').update([
        {
          ...
          event_date: eventDate, // Keep it in the same format
    ...
    return (
      <Box p={5}>
        ...
        <FormControl id='event-date' mt={4} isRequired>
          ...
          <Input
            type='text' // Use text input to accept the "YYYY-MM-DD" format
            value={eventDate}
